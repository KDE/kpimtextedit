...
public:
    ...
    Q_REQUIRED_RESULT const QList<UnicodeEmoticon> &emoticonList() const;
    void setEmoticonList(const QList<UnicodeEmoticon> &newEmoticonList);

private:
    Q_DISABLE_COPY(EmoticonUnicodeModel)
    QList<UnicodeEmoticon> mEmoticonList;
};
}
